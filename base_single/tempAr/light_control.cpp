...
void setSceneMode(const String& mode) {
  ...
  if (mode == "Reading") {
    ...
    /*party频闪
    strobeState = true;
    strobeSpeed = 100;    // 默认速度
    strobeColor = 1;      // 默认使用彩色模式
    strobeColorHue = 0;   // 初始色相
    lastStrobeUpdate = millis();
    
    // 设置初始颜色
    manualRedValue = 0; // 初始为亮灯状态
    manualGreenValue = 0;
    manualBlueValue = 0;
...
void adjustLightingBasedOnSensor() {
  ...
  // 添加阈值判断：当光敏值超过90%时关灯
  const int LIGHT_THRESHOLD = 75;  // 10% of 1023
...
void checkVoiceCommands() {
  ...
  if (Serial.available() >= 4) {
    ...
    if (receivedData[0] == 0xA1 && receivedData[1] == 0xA1 &&
        receivedData[2] == 0xA1 && receivedData[3] == 0xA1) { // 开启卧室灯 (白光)
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xA2 && receivedData[3] == 0xA2) { // 关闭客厅灯
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xA3 && receivedData[3] == 0xA3) { // 亮度调高
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xA4 && receivedData[3] == 0xA4) { // 亮度调低
       if(voiceReddata == voiceGreendata && voiceBluedata == voiceGreendata) { // 灰度/白光
        int currentBrightness = voiceReddata;
    ...
    else if (receivedData[0] == 0xA5 && receivedData[1] == 0xA5 &&
             receivedData[2] == 0xA5 && receivedData[3] == 0xA5) { // 打开柔光灯 (暖光)
      voiceReddata = 102;
      voiceGreendata = 129;
      voiceBluedata = 231;
      voiceCommand = "light Soft";
      Serial.println("语音命令：卧室柔光灯");
      commandProcessed = true;
    }
    else if (receivedData[0] == 0xA6 && receivedData[1] == 0xA6 &&
             receivedData[2] == 0xA6 && receivedData[3] == 0xA6) { // 阅读模式开启 (暖光)
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xA7 && receivedData[3] == 0xA7) { // 阅读模式关闭 (灯灭)
      voiceReddata = 255;
      voiceGreendata = 255;
      voiceBluedata = 255;
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xA8 && receivedData[3] == 0xA8) { // 睡眠模式开启
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xA9 && receivedData[3] == 0xA9) { // 睡眠模式关闭 (灯灭)
      voiceReddata = 255;
      voiceGreendata = 255;
      voiceBluedata = 255;
    ...
    // 添加切换到自动模式的命令
      ...
      commandProcessed = true;
             ...
             receivedData[2] == 0xE5 && receivedData[3] == 0xE5) { // 客厅影视模式（跨房间）
      Serial.println("语音命令：客厅影视模式（跨房间）");
      commandProcessed = true;
    }
       else if (receivedData[0] == 0xE6 && receivedData[1] == 0xE6 &&
             receivedData[2] == 0xE6 && receivedData[3] == 0xE6) { // 关闭客厅影视模式（跨房间）
      Serial.println("语音命令：关闭客厅影视模式（跨房间）");
      commandProcessed = true;
    }
      else if (receivedData[0] == 0xE7 && receivedData[1] == 0xE7 &&
             receivedData[2] == 0xE7 && receivedData[3] == 0xE7) { // 客厅派对模式（跨房间）
      Serial.println("语音命令：打开客厅派对模式（跨房间）");
      commandProcessed = true;
    }
   else if (receivedData[0] == 0xE8 && receivedData[1] == 0xE8 &&
             receivedData[2] == 0xE8 && receivedData[3] == 0xE8) { // 关闭客厅派对模式（跨房间）
      Serial.println("语音命令：关闭客厅派对模式（跨房间）");
      commandProcessed = true;
    }
  else if (receivedData[0] == 0xD3 && receivedData[1] == 0xD3 &&
             receivedData[2] == 0xD3 && receivedData[3] == 0xD3) { // 客厅自动模式（跨房间）
      Serial.println("语音命令：客厅自动模式（跨房间）");
      commandProcessed = true;
    }
  else if (receivedData[0] == 0xD4 && receivedData[1] == 0xD4 &&
             receivedData[2] == 0xD4 && receivedData[3] == 0xD4) { // 客厅手动模式（跨房间）
      Serial.println("语音命令：客厅手动模式（跨房间）");
      commandProcessed = true;
    }
    else {
