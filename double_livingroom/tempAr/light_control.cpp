...
void adjustLightingBasedOnSensor() {
 ...
 void checkVoiceCommands() {
      ...
      if (Serial.available() >= 4) {
        ...
        // 处理模式切换命令
        if (receivedData[0] == 0xD3 && receivedData[1] == 0xD3 &&
            receivedData[2] == 0xD3 && receivedData[3] == 0xD3) {
          Serial.println("语音命令:客厅自动模式");
          switchFromVoiceMode("autoMode");
          return; // 直接返回,因为已经不在语音模式了
        }
        else if (receivedData[0] == 0xD4 && receivedData[1] == 0xD4 &&
                 receivedData[2] == 0xD4 && receivedData[3] == 0xD4) {
          Serial.println("语音命令:客厅手动模式");
        ...
        if (receivedData[0] == 0xA1 && receivedData[1] == 0xA1 &&
          ...
          // 发送到卧室ESP
          sendToPeer("ESP_BEDROOM", jsonCmd);
          Serial.println("语音命令:打开卧室灯(跨房间)");
          ...
          sendToPeer("ESP_BEDROOM", jsonCmd);
          Serial.println("语音命令:卧室睡眠模式(跨房间)");
          commandProcessed = true;
        }
        ...
        // 处理本地房间命令 - 直接控制灯光
          ...
          voiceCommand = "Film model";
          Serial.println("语音命令:客厅影视模式");
          commandProcessed = true;
        }
            ...
            partyModeActive = true;
            partyHue = 0; // 重置色相起点
            voiceCommand = "Party model";
            Serial.println("语音命令:客厅派对模式(彩虹灯效果)");
            commandProcessed = true;
        }
...
void handleRemoteCommand(const String& message) {
  ...
  partyHue = 0; // 重置色相起点
  voiceCommand = "Party model";
  Serial.println("远程命令:激活派对模式(彩虹灯效果)");
  }
